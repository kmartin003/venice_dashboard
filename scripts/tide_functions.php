...
<?
	...
	function colorForTide($tide_level) {
		if ($tide_level >= 1.4) {
			return "Red";
	...
	function highestTideToday() {
		...
		foreach ($tide_measurements as $tide_measurement) {
			...
			if (phpDateTime($date_time) > time() && $max_min == 'massimo' && $tide_level > $max_seen_tide && date('d', phpDateTime($date_time)) == date('d', time())) {									$max_seen_tide = $tide_level;
				$max_seen_time = $time;
			}
		}
